...
class stereoRectify:
    ...
    def stereo_calibrate_video(self, left_video_path, right_video_path):
        # 1. 初始化标定板角点坐标（世界坐标系）
        obj_points = []  # 3D点（棋盘格角点的物理坐标）
        left_img_points = []  # 左图像2D点
        right_img_points = []  # 右图像2D点
    ...
    def stereo_rectify_with_distortion(self, K_left, dist_left, K_right, dist_right, R, T, image_size):
        ...
        R1, R2, P1, P2, Q, roi1, roi2 = cv2.stereoRectify(
            K_left, dist_left, K_right, dist_right, self.image_size, R, T, flags=flags
        )
        # 2. 计算映射表（考虑畸变）
...
if __name__ == "__main__":
    ...
    if method ==1:
        # 图像路径
        img_dir_left = r"data/left"
        img_dir_right = r"data/right"
        img_type = "png"  # 图片格式
        ...
        left_video_path = r"data/20250306134745/step1-3/a0.avi"
        right_video_path = r"data/20250306134745/step1-3/a1.avi"
        ...
        # 矫正视频
        left_video_path = r"data/20250306134745/step2-1/a0.avi"
        right_video_path = r"data/20250306134745/step2-1/a1.avi"
        cap_left = cv2.VideoCapture(left_video_path)
        cap_right = cv2.VideoCapture(right_video_path)
